...
func (qs *QuadStore) createBuckets(ctx context.Context, upfront bool) error {
	err := kv.Update(ctx, qs.db, func(tx kv.Tx) error {
		...
		_ = kv.CreateBucket(ctx, tx, logIndex)
...
func (qs *QuadStore) incNodes(ctx context.Context, tx kv.Tx, deltas []graphlog.NodeUpdate) (map[values.ValueHash]resolvedNode, error) {
	var (
		ins []nodeUpdate
		upd = make([]nodeUpdate, 0, len(deltas))
		ids = make(map[values.ValueHash]resolvedNode, len(deltas))
	)
...
func (qs *QuadStore) decNodes(ctx context.Context, tx kv.Tx, deltas []graphlog.NodeUpdate, nodes map[values.ValueHash]uint64) error {
...
func (qs *QuadStore) ApplyDeltas(in []graph.Delta, ignoreOpts graph.IgnoreOpts) error {
	...
	if len(deltas.QuadDel) != 0 || len(deltas.DecNode) != 0 {
		...
		dnodes := make(map[values.ValueHash]uint64, len(deltas.DecNode))
...
func bucketKeyForVal(v quad.Value) kv.Key {
	hash := values.HashOf(v)
	return bucketKeyForHash(hash)
}

func bucketKeyForHash(h values.ValueHash) kv.Key {
	return bucketForVal(h[0], h[1]).AppendBytes(h[:])
}

func bucketKeyForHashRefs(h values.ValueHash) kv.Key {
